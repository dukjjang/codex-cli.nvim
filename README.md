...
`chat.mcp_auto_approve = { argent = { "list-devices", "list_devices" } }`처럼 서버별 도구 이름을 지정할 수 있습니다. 기본값은 비어 있습니다. 지정한 서버·도구의 표준 `Allow the … MCP server to run tool "…"?` 승인 문구가 정확히 일치하고 추가 입력 없는 빈 `form`일 때만 자동 승인합니다. URL 인증, 입력 필드가 있는 양식, 다른 도구와 서버는 사용자에게 묻습니다.

### MCP 승인·입력 요청

MCP 서버의 `mcpServer/elicitation/request`를 승인 UI로 처리합니다. 서버 이름과 요청 내용을 확인한 뒤 승인·거절할 수 있으며, 취소하거나 종료된 작업의 요청을 뒤늦게 승인하면 `cancel`로 응답합니다. 질문 모드에서도 MCP 요청은 사용자에게 표시되며 자동 승인하지 않습니다.

표준 `form`의 빈 승인 양식, 문자열·숫자·불리언·선택지·문자열 배열 입력을 지원합니다. URL 인증은 표시된 HTTP(S) 주소를 사용자가 선택했을 때만 브라우저에서 엽니다. 입력 내용은 대화 기록에 별도로 저장하지 않습니다. 지원하지 않는 확장 양식은 안내 후 취소합니다.
...
| `Tab` (자동완성 닫힘) | 프롬프트를 큐에 추가; 응답 완료 후 순서대로 자동 전송 |
...
응답 중 후속 프롬프트를 입력하고 Tab을 누르면 대기 목록에 추가됩니다. 여러 요청은 입력 순서대로 현재 응답이 정상 완료된 뒤 전송됩니다. 응답 중이 아니면 바로 전송합니다. 자동완성이 열려 있으면 Tab은 후보를 확정합니다.

자동 전송은 입력창의 새 초안을 지우지 않으며, 등록 당시의 에디터 컨텍스트를 사용합니다. 모드를 변경했다면 등록 당시 모드로 돌아와야 큐를 재개할 수 있습니다. 오류·중단 시 남은 큐는 보존하고 자동 전송을 멈춥니다. 빈 입력창에서 Tab 또는 `:CodexQueue`로 재개하고, `:CodexQueueClear`로 비울 수 있습니다. 큐는 현재 Neovim 세션 메모리에만 보관하며 새 대화를 시작하면 초기화됩니다.
